...
{				// arg[0]: pdev, arg[1]:imx6ul_pinctrl_info / imx6ull_snvs_pinctrl_info
	...
	// ipctl(devm_kzalloc)
	// info->pin_regs(dev_kmalloc)
	
	imx_pinctrl_probe_dt(pdev, info);		//初始化所有funciotn 和group 所有pin 6个值
	
	
	pinctrl_register(imx_pinctrl_desc, &pdev->dev, ipctl);	//注册 pinctrl
...
static int imx6ul_pinctrl_probe(struct platform_device *pdev)
{
	...
	pinctrl_info = (struct imx_pinctrl_soc_info *) match->data;			//获取 match->table
